...
export default function CatalogPage() {
  ...
  const { data, error, isValidating } = useSWR(buildQueryKey(filters), fetcher, { keepPreviousData: true })
  ...
  const products = useMemo(() => (Array.isArray(data) ? data : data?.products || []), [data])
    ...
    <div className="app-shell page">
      <header className="page-header">
        <div>
          ...
          <h1>All products</h1>
          <p className="lede">Filter by category, price range, or search for specific tags and brands.</p>
        </div>
      </header>
      ...
      <section className="products-section">
        <div className="section-header">
          ...
          {pagination && <small>Page {pagination.page} of {pagination.pages}</small>}
        </div>

        {error && <div className="page-state error">Could not load catalog.</div>}
